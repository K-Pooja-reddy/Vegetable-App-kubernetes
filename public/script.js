...
/* this code calculates the total cost of selected vegetables, validates the form, and handles adding items to a cart. 
When the user submits the order, the data is sent to the server, and the cart information is stored in the browser for future reference. */
...
async function calculateTotalCost() {
    ...
    totalCost += (document.getElementById('spinachQty').value || 0) * rates.spinach;
    totalCost += (document.getElementById('corianderQty').value || 0) * rates.coriander;
    totalCost += (document.getElementById('KarvepakuQty').value || 0) * rates.Karvepaku;
    totalCost += (document.getElementById('pudinaQty').value || 0) * rates.pudina;
...
document.getElementById('orderForm').addEventListener('submit', async function(event) {
    ...
    const vegetableQuantities = [
        ...
        'spinachQty', 'corianderQty', 'KarvepakuQty', 'pudinaQty'
    ];
    ...
    const data = {
        ...
        vegetables: {
            ...
            spinachQuantity: parseInt(formData.get('spinachQty') || 0, 10),
            coriander: parseInt(formData.get('corianderQty') || 0, 10),
            Karvepaku: parseInt(formData.get('KarvepakuQty') || 0, 10),
            pudina: parseInt(formData.get('pudinaQty') || 0, 10)
        },
